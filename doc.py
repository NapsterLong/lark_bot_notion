...
def bitable_list_tables(app_token) -> List[AppTable]:
    ...
    # 构造请求对象
    request: ListAppTableRequest = ListAppTableRequest.builder().app_token(app_token).build()
...
def bitable_delete_record(app_token, table_id, record_id):
    ...
    # 发起请求
    response: DeleteAppTableRecordResponse = lark_client.bitable.v1.app_table_record.delete(request)
...
def docx_list_blocks(obj_token) -> List[Block]:
    ...
    # 发起请求
    response: ListDocumentBlockResponse = lark_client.docx.v1.document_block.list(request)
